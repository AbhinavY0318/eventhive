...
function getRandomFutureDate(minDays = 7, maxDays = 90): number {
  const now = Date.now();
  const randomDays =
    Math.floor(Math.random() * (maxDays - minDays)) + minDays;
  return now + randomDays * 24 * 60 * 60 * 1000;
}

function getEventEndTime(startTime: number): number {
...
export const run = internalMutation({
  handler: async (ctx) => {
    ...
    if (!organizer) {
      ...
      organizer = (await ctx.db.get(organizerId)) as Doc<"users">;
    }
    ...
    for (const eventData of SAMPLE_EVENTS) {
      ...
      await ctx.db.insert("events", {
        ...
        locationType: "physical" satisfies LocationType,
        ...
        ticketType: eventData.ticketType as TicketType,
